...
def solution(word):
    ...
    index = 0   # 주어진 단어가 사전에서 몇 번째에 위치하는지를 저장하는 변수

    # word의 각 문자를 순회하면서 해당 문자가 위치한 자리의 가중치를 사용하여 사전적 위치를 계산
    for i, char in enumerate(word):
        char_index = vowels.index(char)         # 현재 문자 char가 vowels에서 몇 번째인지('A'는 0, 'E'는 1)
        index += char_index * weights[i] + 1    # 1을 더하는 이유는 시작 위치가 1이기 때문
